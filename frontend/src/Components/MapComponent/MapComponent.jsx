...
const MapComponent = ({
  ...
  async function fetchParkingSigns(appToken, whereClause) {
    const url = `https://data.cityofnewyork.us/resource/nfid-uabd.json?$where=${encodeURIComponent(
      whereClause
    )}&$limit=5000`;
    ...
    return data
      .map((row) => {
        ...
        if (
          row.sign_description.includes("NO PARKING ANYTIME") ||
          row.sign_description.includes("NO STANDING ANYTIME")
        ) {
          type = "restricted";
        }
        return {
  ...
  useEffect(() => {
    ...
    const fetchSignsInViewport = async () => {
      ...
      const whereClause = `
    sign_x_coord >= ${swSP.x.toFixed(3)} AND sign_x_coord <= ${neSP.x.toFixed(
        3
      )} AND
    sign_y_coord >= ${swSP.y.toFixed(3)} AND sign_y_coord <= ${neSP.y.toFixed(
        3
      )}
    `;
  ...
  return (
    ...
    >
      ...
      {userLocation && (
        <>
          <AdvancedMarker position={userLocation} zIndex={6}>
      ...
      {userLocation && (
        <div className={"pan-to-user-button"}>
          <button
            className={"location-button"}
            onClick={() => {
              smoothPanAndZoom(map, userLocation, 17, 600);
            }}
          >
            Show my location
          </button>
